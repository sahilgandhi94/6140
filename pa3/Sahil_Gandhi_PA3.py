...
if __name__ == '__main__':
    ...
    GMM('data_1_small', 2, covariance_type='full').fit(plot=True, save=True)
    GMM('data_1_large', 2, covariance_type='full').fit(plot=True, save=True)
    ...
    GMM('data_3_small', 3, covariance_type='full').fit(plot=True, save=True)
    GMM('data_3_large', 3, covariance_type='full').fit(plot=True, save=True)

    # print('data 1')
    # GMM.q3_3('data_1_small', 'data_1_large', 10, save=True)
    # print('data 2')
    # GMM.q3_3('data_2_small', 'data_2_large', 10, save=True)
    # print('data 3')
    # GMM.q3_3('data_3_small', 'data_3_large', 10, save=True)
    ...
    m1 = np.loadtxt('data/mystery_1.txt')
    m2 = np.loadtxt('data/mystery_2.txt')
    gm1 = GaussianMixture(1)
    gm1.fit(m1)
    plotMOG(
            m1, [MOG(pi=gm1.weights_[i], mu=gm1.means_[i], var=gm1.covariances_[i]) for i in range(1)], 
            title='Sklearn Gaussian Mixture on mystery 1 data', save=True
        )
    
    gm2 = GaussianMixture(1)
    gm2.fit(m2)
    plotMOG(
            m2, [MOG(pi=gm2.weights_[i], mu=gm2.means_[i], var=gm2.covariances_[i]) for i in range(1)], 
            title='Sklearn Gaussian Mixture on mystery 2 data', save=True
        )
